...
class CriticNetwork(nn.Module):
    def __init__(self, n_states, fc1_dims, fc2_dims, fc3_dims, fc4_dims, n_actions):
        ...
        self.fc1 = nn.Linear(self.n_states + self.n_actions, self.fc1_dims)
    ...
    def forward(self, state, action):
        y = torch.cat([state, action], dim=1)
        y = self.fc1(y)
        y = F.relu(y)
...
class DDPGAgent:
    ...
    def __init__(self, state_dim, action_dim, buffer, beta=0.001, alpha=0.0002,  gamma=0.99, batch_size=128):
        ...
        """
        self.actor_net = ActorNetwork(state_dim, 400, 300, 300, 100, action_dim).to(device)
        self.critic_net = CriticNetwork(state_dim, 400, 300, 300, 100, action_dim).to(device)
